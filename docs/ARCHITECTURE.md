...
- Raw observation persistence derives `sha256:<lowercase-hex>` from exact UTF-8 payload bytes, normalizes instants to PostgreSQL microseconds and uses insert-first `ON CONFLICT DO NOTHING RETURNING`. Equal retries keep the first record; conflicting immutable evidence fails without overwrite. V1 is an ordinary unpartitioned table with only its natural primary-key index: adding `ingested_at` to that key would let later retries bypass global idempotency. No surrogate ID, BRIN/secondary index, normalization or provider transport is part of this storage boundary.
