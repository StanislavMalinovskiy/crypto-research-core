...
- **Текущий этап:** Этап 2 — Первый вертикальный срез.
- **Текущая работа:** реализовать и принять review OpenSpec change `build-first-signal-evaluation-skeleton` с точным контрактом одного end-to-end сценария на записанном входе.
- **Следующая операционная работа:** после принятия review проверить change, синхронизировать accepted specs и выполнить контролируемое архивирование.
...
- **Условие перехода к этапу 3:** одинаковый записанный вход детерминированно создаёт трассируемый сигнал, измеренный результат и одинаковый отчёт без нарушения границ модулей.
...
| 2. Первый вертикальный срез | Current | Один сигнал на записанных данных измеряется от raw input до воспроизводимого отчёта |
...
| 2.1 | Current | Реализовать и принять review OpenSpec change `build-first-signal-evaluation-skeleton` с точным контрактом одного end-to-end сценария; не переводить пакет в Done до решения Control. |
| 2.2 | Next | Пропустить записанный provider fixture через raw replay и нормализовать одно рыночное событие. |
